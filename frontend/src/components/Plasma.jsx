...
void mainImage(out vec4 o, vec2 C) {
  ...
  for (vec2 r = iResolution.xy, Q; ++i < 60.0; O += o.w/d*o.xyz) {
    ...
    z += d = (
      abs(sqrt(length(Q * Q)) - .25 * (5. + S.y)) / 3.
      + 8e-4
    ) * uStepScale;
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uStepScale: {
          value: ORIGINAL_QUALITY / iterations
        }
      }
    });
    ...
    const setSize = () => {
      ...
      const width = Math.max(
        1,
        Math.floor(rect.width * renderScale)
      );

      const height = Math.max(
        1,
        Math.floor(rect.height * renderScale)
      );
